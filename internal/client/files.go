...
//
// When stat fails, the wrapped error lists the valid input forms so the
// caller can self-diagnose. The base error is wrapped so callers using
// errors.Is(err, fs.ErrNotExist) still work.
