...
// formatActionWaitError wraps an error from a client wait helper into the
// message we want to show users of an action verb (extract, classify,
// parse, split, edit, run). The fmt.Errorf "wait: %w" pattern that used
// to live in every caller is preserved for non-timeout errors so unrelated
// failures (transport, API errors, parent cancel) keep their existing
// surface.
//
// For *client.WaitTimeoutError we render an actionable message that names
// the elapsed --timeout and the two recovery paths: raise --timeout, or
// detach with --wait=false and follow up with `extend runs watch <id>`.
// Without this guidance, agents tend to silently retry the same blocking
// command (as observed in the agent-experience transcripts) instead of
// switching to the cheap async path.
func formatActionWaitError(err error, runID string) error {
	if err == nil {
		return nil
	}
	var wt *client.WaitTimeoutError
	if errors.As(err, &wt) {
